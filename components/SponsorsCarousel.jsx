...
function initializeCarousel(root) {
  ...
  const renderDots = () => {
    ...
    dots.innerHTML = Array.from(
      { length: pageCount },
      (_, index) =>
        `<button type="button" class="owl-dot${index === 0 ? " active" : ""}" aria-label="Show sponsors ${index + 1}"><span></span></button>`,
    ).join("");
  };
...
export default function SponsorsCarousel() {
  useEffect(() => {
    const cleanup = [...document.querySelectorAll(".sponsors-carousel")].map(
      initializeCarousel,
    );
